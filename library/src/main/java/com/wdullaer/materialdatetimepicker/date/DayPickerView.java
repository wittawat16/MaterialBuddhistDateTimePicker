...
public abstract class DayPickerView extends RecyclerView implements OnDateChangedListener {
    ...
    void accessibilityAnnouncePageChanged() {
        ...
        if (mv != null) {
            final Locale locale = mController.getLocale();
            String monthYear = getMonthAndYearString(mv.mMonth, mv.mYear, locale);
            if (locale.getLanguage().equalsIgnoreCase(Utils.LOCALE_TH)) {
                monthYear = Utils.getThaiYear(monthYear);
            }
